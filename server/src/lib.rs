//! Server backend for the Collab collaborative text editor.
...
use crate::{database::Database, collab::Collab};
...
async fn socket_handler(id: String, ws: Ws, state: ServerState) -> Result<impl Reply, Rejection> {
    ...
    let mut entry = match state.documents.entry(id.clone()) {
        ...
        Entry::Vacant(e) => {
            let Collab = Arc::new(match &state.database {
                Some(db) => db.load(&id).await.map(Collab::from).unwrap_or_default(),
                None => Collab::default(),
            });
            if let Some(db) = &state.database {
                tokio::spawn(persister(id, Arc::clone(&Collab), db.clone()));
            }
            e.insert(Document::new(Collab))
        }
    };
...
async fn persister(id: String, Collab: Arc<Collab>, db: Database) {
    ...
    while !Collab.killed() {
        ...
        if revision > last_revision {
            ...
            if let Err(e) = db.store(&id, &Collab.snapshot()).await {
